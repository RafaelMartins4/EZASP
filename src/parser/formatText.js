...
function formatText(text) {
	...
	for (i; i < text.length; i++) {
		...
		let a = text[i].trim().indexOf('"');
		let b = text[i].trim().slice(a+1).indexOf('"', 1);
		let c = text[i].trim().indexOf('.');
		if (a!=-1 && b!=-1 && c!= -1 && c<a+b+1)
			text[i] = text[i].trim().replace('.', '');
		if (text[i].trim().includes('%*')) {
		...
		else if (!text[i].trim().includes('.') && text[i].trim() != "") {
			let start = i;
